# -*- coding: utf-8 -*-
"""
Created on Wed Sep 29 20:18:43 2021
...
import glob
import pandas as pd
def plot_aq(fn,fn2,path,allfileimg,percent):
...
#    tit='Vessel'+df_id_name 
    ...
    df=pd.read_csv(fn,sep=',',skiprows=9,parse_dates={'datet(GMT)':[1]},index_col='datet(GMT)',date_parser=parse)#creat a new Datetimeindex
        #df2=pd.read_csv(fn2,sep=',',skiprows=33,parse_dates={'datet(GMT)':[1]},index_col='datet(GMT)',date_parser=parse2)#creat a new Datetimeindex
    df2=read_csv(fn2,sep=',',skiprows=33,parse_dates={'datet':[1]},header=None,index_col='datet',date_parser=parse2)#creat a new Datetimeindex
    #df2.index=df.index+pd.tseries.timedeltas.to_timedelta(4, unit='h')  #, chage it to UTC time
    print (df2)
    #df2=df2.iloc[(df2[9]>0.90*np.mean(df2[9]))]
...
#    df.index=df.index-pd.tseries.timedeltas.to_timedelta(4, unit='h')  #, chage it to UTC time
    ...
    ax1.plot(df.index,df['temp'],'green')
...
#    ax22.set_ylim(min(df['Depth(m)'].values)/1.8288,max(df['Depth(m)'].values)/1.8288)
    ...
    return pic_name
def main():
    fn='D:\\data\\iuion\\towifi\\li_7ad5_20210415_210700.csv'
    fn2='D:\\data\\iuion\\DATA\\2021\\04\\Logger_sn_1724-41_data_20210404_0837321.csv'
    path='D:\\data\\iuion\\towifi'
    allfileimg='0'
    percent=90
    pic_name=plot_aq(fn,fn2,path,allfileimg,percent)

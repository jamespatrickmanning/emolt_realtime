...
def all_boat_map(df,path_save,telemetrystatus_df,emolt_QCed_df_save,start_time,end_time):
...
#    folium.LayerControl().add_to(map)
    ...
    file.close()
    with open(os.path.join(emolt_QCed_df_save,'weekly.html'),'a') as file:  #add the explaintion in html
        file.write('''        <body>
            <div id="header"><br>
                <h3>&nbsp;&nbsp;&nbsp;&nbsp&nbsp;Average position and bottom temperature for each boat reporting in the past week (click icons for data)</h3>  
                <ul>
...
#        CrmClim_df=check_time(dictt['CrmClim'][i],start_time,end_time,time_header='time').dropna()
    ...
    if len(df)!=0:
        ...
        all_boat_map(df,path_save,telemetrystatus_df,emolt_QCed_df_save,start_time,end_time)
        #    for i in df.index:

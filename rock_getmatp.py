...
version 0.0.4 update on 04/25/2021
1, combine rockblock 7 and ap3 together, need to change control file to rock or ap3
2, lower input capital bluetooth case, compatible shallow and short hauls
3,"ap3_test" is compatible rockblock test
            ...
            if valid=='yes':       
                if transmit=='yes':
                        ...
                        try:
                                ...
                                if mode=='real':
                                    if max(df['Depth (m)'])>25:
                                        dft=df.ix[(df['Depth (m)']>0.85*max(df['Depth (m)']))]  # get rid of shallow data
                                        dft=dft.ix[7:]
                                    else:
                                        dft=df.ix[(df['Depth (m)']>0.50*max(df['Depth (m)']))]  # get rid of shallow data
                                        dft=dft.ix[5:]
                                       #delay several minutes to let temperature sensor record the real bottom temp
                                    if boat_type<>'fixed':
